...
class SliceSmsParser(BaseSmsParser):
    ...
    def parse(self, content: str, date_hint: Optional[datetime] = None) -> Optional[ParsedTransaction]:
        # Implementation to be compatible with base class usage
        for pattern in self.get_patterns():
            match = pattern.regex.search(content)
            if match:
                # Extract fields using named groups from regex or the field map if needed
                # My regex uses named groups, so I can extract directly
